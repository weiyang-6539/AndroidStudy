...
/**
 * 通过属性可以指定其子级的布局，例如CSS Flexible Box Layout。
 * 这个布局类似LinearLayout、RelativeLayout一样继承ViewGroup，可以从xml或代码中指定属性
 * 属性详见attrs.xml
 */
public class FlexboxLayout extends ViewGroup {

    @IntDef({FlexDirection.ROW, FlexDirection.ROW_REVERSE,
     ...
     * ROW_REVERSE 由右至左横排
     * COLUMN 由上至下竖排
     * COLUMN_REVERSE 由下至上竖排
     */
    @FlexDirection
    private int mFlexDirection = FlexDirection.ROW;

    @IntDef({FlexWrap.NOWRAP, FlexWrap.WRAP, FlexWrap.REVERSE})
    @Retention(RetentionPolicy.SOURCE)
    ...
    /**
     * 此属性控制flex容器是单行还是多行，以及横轴的方向
     * REVERSE时在原有方向上再次取反
     */
    @FlexWrap
    private int mFlexWrap = FlexWrap.NOWRAP;
    ...
    /**
     * 为子视图缓存{@link LayoutParams＃order}属性。
     * 键：视图索引（不考虑{@link #mReorderedIndices}）
     * 值：订单属性的值
     */
    ...
    /**
     * 在测量期间保持孩子的“冻结”状态。 如果视图被冻结，则无论flexGrow / flexShrink如何，
     * 视图都将不再扩展或缩小。 项目通过孩子的重新排序索引编制索引。
     */
    ...
    public FlexboxLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mFlexDirection = a.getInt(R.styleable.FlexboxLayout_flexDirection, mFlexDirection);
        mFlexWrap = a.getInt(R.styleable.FlexboxLayout_flexWrap, mFlexWrap);
        mJustifyContent = a.getInt(R.styleable.FlexboxLayout_justifyContent, mJustifyContent);
        mAlignItems = a.getInt(R.styleable.FlexboxLayout_alignItems, mAlignItems);
        mAlignContent = a.getInt(R.styleable.FlexboxLayout_alignContent, mAlignContent);
