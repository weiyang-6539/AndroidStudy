    ...
    companion object {
        ...
        fun getMainPageData(page: Int): List<Simple> {
            return when (page) {
                ...
                1 -> mutableListOf(
                    Simple("OkHttp的学习", "okhttp", OkHttpFragment::class.java.simpleName),
                    Simple("Retrofit的学习", "retrofit", RetrofitFragment::class.java.simpleName),
                    Simple("常见加密算法", "常见加密算法的使用", AlgorithmFragment::class.java.simpleName)
                )
                2 -> mutableListOf(
                    Simple("ViewSwitcher", "暂时不知道用法及效果", ""),
                    Simple("Notification", "通知栏的使用", NotificationFragment::class.java.simpleName),
                    Simple("Activity LaunchMode", "", TestLaunchModeFragment::class.java.simpleName)
                )
                3 -> mutableListOf(
                    Simple("自定义控件练习", "五星红旗", CustomViewFragment::class.java.simpleName),
                    Simple("图片九宫格", "仿微信图片九宫格效果", NineGridLayoutFragment::class.java.simpleName),
                    Simple("AppCompatEditText", "仿微博话题效果", WeiBoFragment::class.java.simpleName),
                    Simple("带凸起布局", "仿咸鱼", BottomBarFragment::class.java.simpleName)
                )
                4 -> mutableListOf(
                    Simple(
                        "RecyclerView用法一",
