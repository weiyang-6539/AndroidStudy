    ...
    companion object {
        private val classMap: HashMap<String, Class<*>> = object : HashMap<String, Class<*>>() {
            init {
                ...
                put(MMKVFragment::class.java.simpleName, MMKVFragment::class.java)

                put(NotificationFragment::class.java.simpleName, NotificationFragment::class.java)
                put(TestLaunchModeFragment::class.java.simpleName, TestLaunchModeFragment::class.java)
                put(CustomViewFragment::class.java.simpleName, CustomViewFragment::class.java)
                put(NineGridLayoutFragment::class.java.simpleName, NineGridLayoutFragment::class.java)
                ...
                put(StickyDecorationFragment::class.java.simpleName, StickyDecorationFragment::class.java)
                put(AlbumFragment::class.java.simpleName, AlbumFragment::class.java)
                put(AddressLinkageFragment::class.java.simpleName, AddressLinkageFragment::class.java)
        ...
        fun getMainPageData(page: Int): List<Simple> {
            ...
            when (page) {
                0 -> {
                    data.add(Simple("HandlerThread", "HandlerThread的用法", HandlerThreadFragment::class.java.simpleName))
                    data.add(Simple("IntentService", "IntentService的用法", IntentServiceFragment::class.java.simpleName))
                ...
                1 -> {
                    ...
                    data.add(Simple("MMKV使用","SP替代",MMKVFragment::class.java.simpleName))
                }
                2 -> {
                    data.add(Simple("ViewSwitcher", "暂时不知道用法及效果", ""))
                    data.add(Simple("Notification", "通知栏的使用", NotificationFragment::class.java.simpleName))
                    data.add(Simple("Activity LaunchMode", "", TestLaunchModeFragment::class.java.simpleName))
                }
                3 -> {
                    data.add(Simple("自定义控件练习", "五星红旗", CustomViewFragment::class.java.simpleName))
                    data.add(Simple("图片九宫格", "仿微信图片九宫格效果", NineGridLayoutFragment::class.java.simpleName))
                }
                4 -> {
                    data.add(Simple("RecyclerView用法一", "实现今日头条拖拽排序", DragSortFragment::class.java.simpleName))
                    data.add(Simple("RecyclerView用法二", "仿微信发送图片朋友圈九宫格效果", WeChatFragment::class.java.simpleName))
                    data.add(Simple("RecyclerView用法三", "仿微信通讯录效果", ContactsFragment::class.java.simpleName))
                    data.add(Simple("RecyclerView用法四", "悬浮分组展示", StickyDecorationFragment::class.java.simpleName))
                    data.add(Simple("RecyclerView用法五", "仿小米云相册", AlbumFragment::class.java.simpleName))
                }
                5 -> {
                    data.add(Simple("树结构之省市区镇四级联动", "利用TreeNode及NodeSeeker实现的四级联动", AddressLinkageFragment::class.java.simpleName))
                    data.add(Simple("树结构之筛选遍历", "利用TreeNode及NodeSeeker实现复杂筛选条件,代码简洁的遍历", AddressFilterFragment::class.java.simpleName))
                }
            }
            return data
        }
